...
void drawGridAndTravelersASCII(int**grid, int numRows, int numCols, std::vector<TravelerInfo>& travelerList) {
	// redraw grid
	for (int row = 0; row < numRows; row++) {
        for (int col = 0; col < numCols; col++) {
			bool travelerFound = false;
			// is there a traveler in this grid spot?
			for (auto traveler : travelerList) {
				if (traveler.isLive) {
					if (row == traveler.row && col == traveler.col) {
						// BUG high grid count: extra vertical lines, but no extra horizontal
						switch ((int)(traveler.type)) {
							case RED_TRAV:
								printCell(TextColor::RED, "[", iconDirections[traveler.dir], "]");
								travelerFound = true;
								break;
							case GREEN_TRAV:
								printCell(TextColor::GREEN, "[", iconDirections[traveler.dir], "]");
								travelerFound = true;
								break;
							case BLUE_TRAV:
								printCell(TextColor::BLUE, "[", iconDirections[traveler.dir], "]");
								travelerFound = true;
								break;
						}
					}  
...
void myKeyboard(unsigned char c) {
	...
	switch (c) {
        ...
        // make travelers move faster
        case '+':
            if (travelerSleepTime - 100000 > 0) {
                travelerSleepTime -= 100000;
            }
            break;
        
        // make travelers move slower
        case '-':
            if (travelerSleepTime + 100000 < 2147483647) {
                travelerSleepTime += 100000;
            }
            break;
