...
void threadFunction(TravelerInfo* traveler);
void getNewDirection(TravelerInfo* traveler);
bool checkIfInCorner(TravelerInfo* traveler);
void redColorThreadFunc();
void greenColorThreadFunc();
void blueColorThreadFunc();
bool checkEnoughInk(TravelerInfo* traveler, int moveAmount);
...
// traveler sleep time (in microseconds)
int travelerSleepTime = 1000000; // 1000000
...
void displayGridPane(void) {
	...
	//---------------------------------------------------------
    drawGridAndTravelersASCII(grid, NUM_ROWS, NUM_COLS, info);
}
...
int main(int argc, char** argv) {
    ...
    try {
        // check that arguments are valid, must be a 20x20 or greater and at least 8 threads/travelers
        if (argc == 4) {
        ...
        // TODO: create an almost even amount of producer threads for each ink tank


        // TODO: create producer threads that check the levels of each ink;

        
        // TODO: create threads for the travelers
...
void initializeApplication(void) {
	...
	//	travelers at random locations on the grid:
	//		- not at a corner
    ...
    TravelerInfo trav = {	RED_TRAV,
							1, 5,
							EAST, true};
	info.push_back(trav); // aka the travelers
}
...
// TODO check if you have enough ink depending on what kind of traveler you are, maybe some locks/unlocks
bool checkEnoughInk(TravelerInfo* traveler, int moveAmount) {
